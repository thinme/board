...
function getReplyList(data){
	...
	for(var i=0; i<replyList.length; i++){
      ...
      replyListHtml.append($('<input/>', {type: "hidden", name:"rno", value:replyList[i].rno}))
             .append("<div>")
             .append('<div style="text-align:left; padding-left: 15px"> 작성자 : '+replyList[i].replyer+ ' //등록일 : '+replyList[i].regDate+' </div>')
             ...
             .append('<div style="text-align:left; padding-left: 15px; border-bottom: 1px solid">' +replyList[i].replyText + ' </div>')
             .append("</div>");
    }else{
      replyListHtml.append($('<input/>', {type: "hidden", name:"rno", value:replyList[i].rno}))
            .append("<div>")
	...
	for(var i=0; i<replyList.length; i++){
		if (replyList[i].rno === rno) {
			replyListHtml.append($('<input/>', {type: "hidden", name:"rno", value:replyList[i].rno}))
				 ...
				 .append('<textarea id="modifyText" style="text-align:left; padding-left: 15px; border-bottom: 1px solid">' +replyList[i].replyText + ' </textarea>')
				 .append("</div>");
		}else{
			replyListHtml.append($('<input/>', {type: "hidden", name:"rno", value:replyList[i].rno}))
             .append("<div class='reple'>")
             .append('<div style="text-align:left; padding-left: 15px"> 작성자 : '+replyList[i].replyer+ '//등록일 : '+replyList[i].regDate+' </div>')
             .append('<button style="float:right; border-radius: 10px; border: 5px solid #F4E8F6; background-color:white; cursor:pointer" onclick="repDelBtn('+replyList[i].rno+')">삭제하기</button>')
             ...
             .append('<div style="text-align:left; padding-left: 15px; border-bottom: 1px solid">' +replyList[i].replyText + ' </div>')
             //.append('<button onclick="repUpBtn('+replyList[i].rno+ ',' +replyList[i].replyText+')">수정하기</button>')
             //.append($('<input/>', {type: "text", id:"text", value:text}))
             //.append
             .append("</div>");
		}
